...
function checkForMatch() {
  ...
  if (firstCard.dataset.framework === secondCard.dataset.framework) {
    ...
    scoreRating(); /////////////// Add score function
  } else {
    //De matchar inte!
...
function stopClock() { // Skapa en funktion som stoppar räkna ut tiden
  clearInterval(clockId);
  clockOff = true;
}

function resetTimer() { // Skapa en funktion som reset tiden till 0
  stopClock();
  clockOff = true;
  time = 0;
  displayTime();
...
function addMoves() { // Räkna spelarens drag när hen öppnar korten
  moves++;
...
function scoreRating(){ // Skapa en funktion som lägger till 3 poäng om korten matchas

  if(moves == 5 || moves == 10 || moves == 15 || moves == 20 || moves == 25 || moves == 30 || moves == 35 || moves == 40 || moves == 45 || moves == 50 || moves == 55 || moves == 60 || moves == 65 || moves == 70 || moves == 75 || moves == 80 || moves == 85 || moves == 90 || moves == 95 || moves == 100){
    score -= 1
  }
...
function losingPoints(){ // Skapa en funktion som tar bort en poäng för varje 5 drag
...
function resetScore(){ // Skapa en funktion som reset poäng till 0
  score = 0;
  playerScore.innerHTML = score;
